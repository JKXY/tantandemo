...
public class RecyclerBanner extends FrameLayout {
    ...
    public RecyclerBanner(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        defaultDrawable.setColor(0xffefefef);
    ...
    public int setDatas(List<BannerEntity> datas){
        ...
        if(this.datas.size() > 1){
            ...
            for (int i = 0; i < this.datas.size(); i++) {
                ...
                if(datas.get(i % datas.size()).getUrl() != null) {
//                    Glide.with(img.getContext()).load(datas.get(i % datas.size()).getUrl()).placeholder(R.drawable.loading).into(img);
                } else if(datas.get(i % datas.size()).getBitmap() != null){
//                    Glide.with(img.getContext()).load(datas.get(i % datas.size()).getBitmap()).placeholder(R.drawable.loading).into(img);
//                    img.setImageBitmap(datas.get(i % datas.size()).getBitmap());
...
//            ImageView img = new ImageView(parent.getContext());
        ...
        public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
            SimpleDraweeView img = (SimpleDraweeView) holder.itemView.findViewById(R.id.icon);
            if(datas.get(position % datas.size()).getUrl() != null) {
                img.setImageURI(datas.get(position % datas.size()).getUrl());
            } else if(datas.get(position % datas.size()).getBitmap() != null){
                img.setImageBitmap(datas.get(position % datas.size()).getBitmap());
            }
        }
